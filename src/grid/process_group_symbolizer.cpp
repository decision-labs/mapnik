...
struct thunk_renderer : render_thunk_list_dispatch
{
    ...
    virtual void operator()(vector_marker_render_thunk const& thunk)
    {
        ...
        using scanline_renderer_type = agg::renderer_scanline_bin_solid<grid_renderer_base_type>;

        using namespace mapnik::svg;
        using svg_renderer_type = svg_renderer_agg<svg_path_adapter,
                                                   svg_attribute_type,
                                                   scanline_renderer_type,
                                                   pixfmt_type>;
...
{
    METRIC_UNUSED auto t = this->metrics_.measure_time("Grid_PGroupS");
